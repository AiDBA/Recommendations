...
Date: 5/18/2023 5:56:50 AM UTC
*/
BEGIN TRY     CREATE NONCLUSTERED INDEX AIDBA_NCIX_230518055653 ON [OCS].[DBO].[AAZA_LOADINGAPPOINTMENTTIME_LINKAGE] ([LAT_ROWID],[T1_TABLENAME]) INCLUDE ([T1_ROWID]) ;
...
BEGIN TRY     CREATE  INDEX [IX_AIDBA_IX_LATESTOBJECTPROPERTY_PROPERTYID] ON [VEEAMONE].[REPORTER].[LATESTOBJECTPROPERTY] ([PROPERTYID],[OBJECTTYPEID]) INCLUDE ([VALUE]) WITH (DROP_EXISTING=OFF );
 END TRY BEGIN CATCH END CATCH;
BEGIN TRY     CREATE NONCLUSTERED INDEX AIDBA_NCIX_230518055705 ON [VEEAMONE].[MONITOR].[BPJOBSESSION] ([UID],[BP_SERVER_ID],[AGG_DURATION],[DELETE_TIME]) ;
 END TRY BEGIN CATCH END CATCH;
