...
Date: 6/19/2023 7:56:24 PM UTC
*/
BEGIN TRY     CREATE NONCLUSTERED INDEX AIDBA_NCIX_230619195626 ON [OCS].[DBO].[A_WILORRYUNLOADINGTRANSACTION] ([TRANSSTATUS],[ROWID]) INCLUDE ([CREATEDBY],[CREATEDDATE],[UPDATEDBY],[UPDATEDDATE],[LORRYNO],[REMARKS],[UNLOADFINISH],[ISLINKTOLOADING]) ;
...
BEGIN TRY     CREATE UNIQUE INDEX [IX_AIDBA_IX_SLIST_UNIQUE] ON [COLOS].[DBO].[SLIST] ([INTVALUE],[STRLISTNAME]) INCLUDE ([STRDESCRIPTION]) WITH (DROP_EXISTING=OFF );
 END TRY BEGIN CATCH END CATCH;
BEGIN TRY     CREATE NONCLUSTERED INDEX AIDBA_NCIX_230619195631 ON [VEEAMONE].[MONITOR].[BPJOBSESSION] ([UID],[BP_SERVER_ID],[AGG_DURATION],[DELETE_TIME]) ;
 END TRY BEGIN CATCH END CATCH;
