...
Date: 7/13/2023 7:30:31 AM UTC
*/
BEGIN TRY     CREATE NONCLUSTERED INDEX AIDBA_NCIX_230713073035 ON [OCS].[DBO].[CCZA_LOADINGAPPOINTMENTTIME_LINKAGE_AGT] ([LAT_ROWID],[T1_TABLENAME],[T1_ROWID]) ;
...
BEGIN TRY     CREATE  INDEX [IX_AIDBA_IX_ENTITYID] ON [VEEAMONE].[REPORTING].[OBJECTENTITYRELATIONS] ([ENDDATE],[ENTITYID],[ENTITYPARENTID],[STARTDATE]) INCLUDE ([ENTITYPARENTTYPEID],[ENTITYPROPERTYTYPEID],[ENTITYTYPEID],[ISDELETED]) WITH (DROP_EXISTING=OFF );
 END TRY BEGIN CATCH END CATCH;
BEGIN TRY     CREATE NONCLUSTERED INDEX AIDBA_NCIX_230713073040 ON [VEEAMONE].[MONITOR].[BPEXTERNALPLATFORM] ([UID],[BP_SERVER_ID],[PLATFORM_ID],[PLATFORM_TASK],[DELETE_TIME],[DISPLAY_NAME]) ;
 END TRY BEGIN CATCH END CATCH;
