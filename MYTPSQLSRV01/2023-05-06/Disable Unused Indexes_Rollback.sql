...
BEGIN TRY
	...
	ON [VeeamOne].[monitor].[Alarm] REBUILD WITH (SORT_IN_TEMPDB = ON);
	PRINT 'INDEX [Unique_Alarm_uid] ON [VeeamOne].[monitor].[Alarm] IS ENABLED.';
END TRY
BEGIN CATCH
	PRINT 'INDEX [Unique_Alarm_uid] ON [VeeamOne].[monitor].[Alarm] CANNOT BE ENABLED.';
END CATCH;
