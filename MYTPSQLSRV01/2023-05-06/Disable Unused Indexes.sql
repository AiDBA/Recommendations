...
BEGIN TRY
	IF EXISTS(  SELECT 1 FROM SYS.INDEXES I WHERE I.[IS_PRIMARY_KEY] = 1 AND I.[NAME] = 'Unique_Alarm_uid')
	BEGIN
	ALTER INDEX [Unique_Alarm_uid]
	ON [VeeamOne].[monitor].[Alarm] DISABLE;
	PRINT 'INDEX [Unique_Alarm_uid] ON [VeeamOne].[monitor].[Alarm] IS DISABLED.';
	END
END TRY
BEGIN CATCH
	PRINT 'INDEX [Unique_Alarm_uid] ON [VeeamOne].[monitor].[Alarm] CANNOT BE DISABLED.';
END CATCH;
