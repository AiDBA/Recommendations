...
BEGIN TRY
	IF EXISTS(  SELECT 1 FROM SYS.INDEXES I WHERE I.[IS_PRIMARY_KEY] = 1 AND I.[NAME] = 'IX_SetPoint_RunID_RunCycleNumber')
	BEGIN
	ALTER INDEX [IX_SetPoint_RunID_RunCycleNumber]
	ON [FusionOperational].[dbo].[SetPoint] DISABLE;
	PRINT 'INDEX [IX_SetPoint_RunID_RunCycleNumber] ON [FusionOperational].[dbo].[SetPoint] IS DISABLED.';
	END
END TRY
BEGIN CATCH
	PRINT 'INDEX [IX_SetPoint_RunID_RunCycleNumber] ON [FusionOperational].[dbo].[SetPoint] CANNOT BE DISABLED.';
END CATCH;
