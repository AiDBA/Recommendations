...
BEGIN
	BEGIN TRY
		IF EXISTS (SELECT 1 FROM [FusionOperational].sys.stats S CROSS APPLY [FusionOperational].sys.dm_db_stats_properties(S.object_id,S.stats_id) P WHERE S.NAME = 'IX_AIDBA_2_20220820' AND OBJECT_NAME(S.OBJECT_ID) = 'AutomaticPrintEvent' AND P.last_updated <= DATEADD(HOUR,-48,SYSDATETIME()))
		BEGIN
			UPDATE STATISTICS [FusionOperational].[dbo].[AutomaticPrintEvent] [IX_AIDBA_2_20220820] WITH FULLSCAN ;
		END
		PRINT '[IX_AIDBA_2_20220820] on [FusionOperational].[dbo].[AutomaticPrintEvent] is updated.';
	END TRY
	BEGIN CATCH
		PRINT '[IX_AIDBA_2_20220820] on [FusionOperational].[dbo].[AutomaticPrintEvent] cannot be updated.';			
	END CATCH;
END
