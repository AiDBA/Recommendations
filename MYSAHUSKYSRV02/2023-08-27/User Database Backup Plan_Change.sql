...
DECLARE @jobId BINARY(16);
	...
	USE [msdb]
	IF EXISTS (SELECT * FROM sys.databases WHERE name = 'ShotscopeNXSecurity' and recovery_model = 1)
	BEGIN
	BEGIN TRANSACTION
	/****** Object:  Job [AIDBA ShotscopeNXSecurity Full Backup]    Script Date: 03/01/2019 11:03:37 AM ******/
	SELECT @ReturnCode = 0
	/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 03/01/2019 11:03:37 AM ******/
	IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
	BEGIN
	...
	SELECT @jobId = NULL
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'AIDBA ShotscopeNXSecurity Full Backup', 
			@enabled=1, 
			@notify_level_eventlog=0, 
			@notify_level_email=0, 
			@notify_level_netsend=0, 
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Database Integrity & Full Backup', 
			...
			@os_run_priority=0, @subsystem=N'TSQL', 
			@command=N'DECLARE @Disk_Full nvarchar(max);
	DECLARE @DB_Name sysname=''ShotscopeNXSecurity'';
	DECLARE @stmt nvarchar(max);
	SELECT @Disk_Full = N''C:\AIDBA_Backups\Full\''+@DB_Name+''_FULL_''+Convert(Nvarchar,SysDateTime(),112)+''.bak'';
	...
	/****** Object:  Job [AIDBA ShotscopeNXSecurity Log Backup]    Script Date: 03/01/2019 11:06:51 AM ******/
	...
	SELECT @jobId = NULL
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'AIDBA ShotscopeNXSecurity Log Backup', 
			@enabled=1, 
			@notify_level_eventlog=0, 
			@notify_level_email=0, 
			@notify_level_netsend=0, 
