...
BEGIN
	BEGIN TRY
		IF EXISTS (SELECT 1 FROM [FusionDW].sys.stats S CROSS APPLY [FusionDW].sys.dm_db_stats_properties(S.object_id,S.stats_id) P WHERE S.NAME = 'IX_AIDBA_1_20220820' AND OBJECT_NAME(S.OBJECT_ID) = 'FactDowntime' AND P.last_updated <= DATEADD(HOUR,-48,SYSDATETIME()))
		BEGIN
			UPDATE STATISTICS [FusionDW].[dbo].[FactDowntime] [IX_AIDBA_1_20220820] WITH FULLSCAN ;
		END
		PRINT '[IX_AIDBA_1_20220820] on [FusionDW].[dbo].[FactDowntime] is updated.';
	END TRY
	BEGIN CATCH
		PRINT '[IX_AIDBA_1_20220820] on [FusionDW].[dbo].[FactDowntime] cannot be updated.';			
	END CATCH;
END
