...
DECLARE @jobId BINARY(16);
	...
	DECLARE @stmt nvarchar(max);
	SELECT @Disk_Full = N''C:\AIDBA_Backups\Full\''+@DB_Name+''_FULL_''+Convert(Nvarchar,SysDateTime(),112)+''.bak'';
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Purge Old Log Backup', 
			...
			@os_run_priority=0, @subsystem=N'PowerShell', 
			@command=N'Set-Location C:
	$TargetPath = "C:\AIDBA_Backups\Full\"
	$Days = 8
	$Extension = "*.trn"
	$CurrentDate = Get-Date
	$LastWrite = $CurrentDate.AddDays(-$days)
	...
	-- path for Backup 
		SELECT @Disk = N''C:\AIDBA_Backups\Full\''+@DB_Name+''_Log_''+Convert(Nvarchar,SysDateTime(),112)+''.trn''
