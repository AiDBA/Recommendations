...
RECONFIGURE WITH OVERRIDE;
EXEC xp_cmdshell 'md G:\AIDBA_Backups\Simple';
EXEC xp_cmdshell 'md G:\AIDBA_Backups\Full';
...
DECLARE @jobId BINARY(16);
	...
	BEGIN
		...
		-- path for Backup
		SELECT @Disk_Full = N''G:\AIDBA_Backups\Simple\''+@DB_Name+''_FULL_''+Convert(Nvarchar,SysDateTime(),112)+''.bak''
		BEGIN TRY
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Purge Old Backup Files', 
			...
			@command=N'Set-Location G:
	# target path
	$TargetPath = "G:\AIDBA_Backups\Simple\"
	...
	DECLARE @stmt nvarchar(max);
	SELECT @Disk_Full = N''G:\AIDBA_Backups\Full\''+@DB_Name+''_FULL_''+Convert(Nvarchar,SysDateTime(),112)+''.bak'';
	...
	EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Purge Old Log Backup', 
			...
			@os_run_priority=0, @subsystem=N'PowerShell', 
			@command=N'Set-Location G:
	$TargetPath = "G:\AIDBA_Backups\Full\"
	$Days = 8
	$Extension = "*.trn"
	$CurrentDate = Get-Date
	$LastWrite = $CurrentDate.AddDays(-$days)
	...
	-- path for Backup 
		SELECT @Disk = N''G:\AIDBA_Backups\Full\''+@DB_Name+''_Log_''+Convert(Nvarchar,SysDateTime(),112)+''.trn''
