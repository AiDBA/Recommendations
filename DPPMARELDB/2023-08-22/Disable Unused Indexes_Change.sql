...
BEGIN TRY
	IF EXISTS(  SELECT 1 FROM SYS.INDEXES I WHERE I.[IS_PRIMARY_KEY] = 1 AND I.[NAME] = 'ix_proc_items_prday')
	BEGIN
	ALTER INDEX [ix_proc_items_prday]
	ON [innova].[dbo].[proc_items] DISABLE;
	PRINT 'INDEX [ix_proc_items_prday] ON [innova].[dbo].[proc_items] IS DISABLED.';
	END
END TRY
BEGIN CATCH
	PRINT 'INDEX [ix_proc_items_prday] ON [innova].[dbo].[proc_items] CANNOT BE DISABLED.';
END CATCH;

BEGIN TRY
	IF EXISTS(  SELECT 1 FROM SYS.INDEXES I WHERE I.[IS_PRIMARY_KEY] = 1 AND I.[NAME] = 'ix_proc_items_plot_rtype')
	BEGIN
	ALTER INDEX [ix_proc_items_plot_rtype]
	ON [innova].[dbo].[proc_items] DISABLE;
	PRINT 'INDEX [ix_proc_items_plot_rtype] ON [innova].[dbo].[proc_items] IS DISABLED.';
	END
END TRY
BEGIN CATCH
	PRINT 'INDEX [ix_proc_items_plot_rtype] ON [innova].[dbo].[proc_items] CANNOT BE DISABLED.';
END CATCH;
